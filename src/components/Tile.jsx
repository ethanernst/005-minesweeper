...
function Tile({ tileSize, scale, row, column }) {
  const { getTileValue } = useContext(GlobalContext);

  const [cellValue, setCellValue] = useState(getTileValue(row, column));
  ...
  useEffect(() => {
    setCellValue(getTileValue(row, column));
  }, [getTileValue]);

  return (
    <TileContainer $imgUrl={imageUrl} $size={SIZE} onClick={handleClick}>
      <img src={imageUrl} />
    </TileContainer>
  );
}
